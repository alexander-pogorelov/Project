		...
		try {
			...
			foreach ($this->routes as $uriPattern => $path){
				...
				if (preg_match($uriPattern, $uri)){// СОВПАДЕНИЕ ТАКЖЕ  НАХОДИТСЯ, ЕСЛИ КЛЮЧЕВОЕ СЛОВО НАДОДИТСЯ НЕ СНАЧАЛА URI
					...
					/*
					if (call_user_func_array(array($controller, $actionName), $parameters)){ // если нужный метод нужного контроллера найден - прерываем цикл поиска
						//echo "Найдено совпадение";
						break;
					}
					else {
						throw new Exception('Станица не найдена');//генерируем исключение
					}
					*/
				}
			}
